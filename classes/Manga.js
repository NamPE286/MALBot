...
class Manga {
    ...
    async init() {
        const res = await fetch(`https://api.jikan.moe/v4/manga/${this.id}`)
        if (!res.ok) {
            throw new Error('Invalid MAL manga\'s ID')
        }
    ...
    getSimplifiedGenres() {
        if (!this.data) {
            throw new Error('Manga data is not initialized')
        }
    ...
    getDiscordEmbed() {
        ...
        return new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: 'Published', value: String(this.data.published.string) },
                {
                    name: 'Authors',
                    value: (() => {
                        var res = []
                        for (const i of this.data.authors) {
                            res.push(i.name)
                        }
                        return res.join(', ')
                    })()
